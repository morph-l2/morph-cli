...
import { MORPH_MAINNET, ALT_FEE_TOKEN_REGISTRY, ALT_FEE_TOKENS } from '../lib/utils/config.js'
...
function explorerCommand(): Command {
  ...
  function apiBase(): string {
    return MORPH_MAINNET.blockscoutApi
  }
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        const data = await httpGet(`${apiBase()}/addresses/${opts.address}`)
        out(true, data)
      } catch (e) { out(false, { error: (e as Error).message }); process.exit(1) }
    })
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        const data = await httpGet<{ items: unknown[] }>(`${apiBase()}/addresses/${opts.address}/transactions`)
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        const data = await httpGet(`${apiBase()}/addresses/${opts.address}/token-balances`)
        out(true, data)
      } catch (e) { out(false, { error: (e as Error).message }); process.exit(1) }
    })
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        const data = await httpGet(`${apiBase()}/transactions/${opts.hash}`)
        out(true, data)
      } catch (e) { out(false, { error: (e as Error).message }); process.exit(1) }
    })
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        const data = await httpGet(`${apiBase()}/tokens`, { q: opts.query })
        out(true, data)
      } catch (e) { out(false, { error: (e as Error).message }); process.exit(1) }
    })
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        const data = await httpGet<Record<string, unknown>>(`${apiBase()}/smart-contracts/${opts.address}`)
        out(true, {
...
function altfeeCommand(): Command {
  ...
  async function registryCall(data: string): Promise<string> {
    const client = getPublicClient()
  ...
  async function getTokenFeeParams(tokenId: number) {
    const idHex = tokenId.toString(16).padStart(64, '0')
    const [infoHex, ratioHex] = await Promise.all([
      registryCall(TR_GET_TOKEN_INFO + idHex),
      registryCall(TR_PRICE_RATIO + idHex),
    ])
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        const hex = await registryCall(TR_GET_TOKEN_LIST)
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        ...
        const info = await getTokenFeeParams(tokenId)
        out(true, {
          tokenId,
...
function eip7702Command(): Command {
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        const info = await getDelegate(opts.address as `0x${string}`)
        out(true, info)
      } catch (e) { out(false, { error: (e as Error).message }); process.exit(1) }
    })
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        ...
        const auth = await signAuth(w, opts.delegate)
        out(true, auth)
      } catch (e) { out(false, { error: (e as Error).message }); process.exit(1) }
    })
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        ...
        }], { delegate: opts.delegate, dryRun: opts.dryRun })
        out(true, result)
      } catch (e) { out(false, { error: (e as Error).message }); process.exit(1) }
    })
  ...
  cmd
    ...
    .action(async (opts) => {
      try {
        const { wallet } = resolveWallet(opts)
        const result = await revoke7702(wallet, { dryRun: opts.dryRun })
        out(true, result)
      } catch (e) { out(false, { error: (e as Error).message }); process.exit(1) }
    })
